...
class DryRunDigitalOceanSpacesClient(
    ...
    override fun uploadFile(file: File) {
        if (!file.exists()) return logger.warn("  | [WARN] File ${file.name} does not exist, skipping upload")
...
class DefaultDigitalOceanSpacesClient(
    ...
    override fun uploadFile(file: File) {
        ...
        try {
            client.use {
                if (!file.exists()) return@use logger.warn("  | [WARN] File ${file.name} does not exist, skipping upload")

                val key = "${ext.artifactPath ?: ""}/${file.name}"

                logger.lifecycle("  | [INFO] Uploading ${file.name} to ${ext.bucket}/$key")
